...
def main() -> None:
    ...
    payload = {
        ...
        "evaluation": (
            "행을 생산시각 순으로 놓고 각 블록을 그 직전까지의 데이터로만 학습한 모델로 예측했다. "
            "모든 행이 같은 평가 집합(4,730건 / 불량 43건) 기준이라 주기 간 비교가 가능하다."
        ),
        "inspect_ratio": INSPECT_RATIO,
        "note": (
            "재학습 주기는 검사 건수가 아니라 생산 샷 단위다. 50샷이 약 25분에 해당한다. "
            "구간 안의 차이(AP 0.071~0.078)는 노이즈이므로 특정 주기 하나를 최적으로 읽으면 안 된다. "
            "의미 있는 구분은 25분 이내 / 1시간~35시간 / 하루 세 구간이다."
        ),
        "best_band": "25분 이내",
        "rows": rows,
    }
