...
# 시연 시작 시 미리 재생해둘 건수.
#
# 위험 제품은 표본 전반에 고르게 있지 않다. 1~100번째는 0건, 301~500번째는
# 200건 내리 0건이고, 501번째부터 500건에 83건이 몰려 있다.
#
# 560을 고른 기준은 세 가지다.
#   1. 검사 대기열이 21건이라 자동 재학습 기준(5건)을 여유 있게 채운다.
#   2. 추이 차트는 최근 60건(501~560)을 그리는데, 이 구간에 threshold를 넘는
#      점이 7개 있어 차트가 바닥에 붙은 평평한 선으로 보이지 않는다.
#      500에서 시작하면 441~500번째가 전부 위험 0건 구간이라 차트가 비어 보인다.
#   3. 남은 재생이 440건(약 7분 20초)이라 발표 길이에 맞는다.
DEMO_SEED_RECORDS = 560
